...
Page({
  ...
  toUserinfo: function (options){
    var that = this;
    wx.navigateTo({
      url: '../usercardinfo/usercardinfo?userid=' + options.currentTarget.dataset.uid
    })
  },
  storelist:function () {
    var that = this;
    var data={};
    var url = "http://api.ispa.local/api/Usercard/storelist";//门店列表
  ...
  userlist: function (data) {
    var that = this;
    var url = "http://api.ispa.local/api/Usercard/userlist";//我的会员
    app.post(url, data).then((res) => {
      that.setData({
        list:res
      }, function () {
