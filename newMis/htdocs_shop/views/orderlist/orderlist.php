...
<div class="weui-tab" id="tab_switch">
    ...
    <div class="weui-navbar">
        <div class="weui-navbar__item" onclick="searchlist('cashier')"><span>消费订单</span></div>
        <div class="weui-navbar__item" onclick="searchlist('charge')"><span>充值订单</span></div>
    </div>
    <!--</div>-->
    <div class="weui-tab__panel">
        <div class="weui-tab__content">
            <div class="con_box">
                <div class="orderlist_date_select" style="display:none;">
                    ...
                    至
                    <input type="text" class="date_end" value="<?php echo date('Y-m-d')?>"  readonly = "readonly">
                </div>
                <!-- Single button -->
                <div class="btn-group">
                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span class="date_txt">今天</span>
                ...
                <div class="orderlist_select_input">
                    <input type="text" id="condition1" placeholder="订单号或手机号">
                    <a href="javascript:;" onclick="searchlist()"></a>
                </div>
            </div>
        ...
        <div class="weui-tab__content rechargelist">
            <div class="con_box">
                <div class="orderlist_date_select" style="display:none;">
                    <i></i>
                    <input type="text" placeholder="开始时间" class="date_begin" readonly = "readonly">
                ...
                <div class="orderlist_select_input">
                    <input type="text" id="condition2" placeholder="订单号或手机号">
                    <a href="javascript:;" onclick="searchlist()"></a>
                </div>
            </div>
            <div class="order_list">
...
<script>
    ...
    $(function(){
        ...
        $(".dropdown-menu>li>a").click(function(){
           ...
           $(".dropdown-toggle").html( '<span class="date_txt">' + menu_text + '</span><span class="caret"></span>'); 
            ...
            if(menu_text == '自定义时间')
            {
                $(".orderlist_date_select").slideDown();
            }
            else
            {
                $(".orderlist_date_select").slideUp();
            }
        });
    ...
    function searchlist(qtype = null) {

        $('.order_list div').remove();

        var role = $("#role").val();
        var userid = $("#userid").val();
        var time = $(".btn-group button .date_txt").html();
        var type = $('.weui-bar__item_on span').html();
        if(qtype == 'cashier'){
            type = 1;
        }else if(qtype == 'charge'){
            type = 2;
        }
        if(type == '消费订单'){
            type = 1;
        ...
        var url = "<?php echo site_url('orderlist/orderList'); ?>";
         data={
            'type': type,
            'condition': condition,
            'userid':  userid,
        ...
        $.ajax({
            ...
            success: function (msg) {
                if(msg.msg == 'success'){
                    $('.order_list div').remove();
                    $('.order_list').append(msg.data);
                }else{
                    $('.order_list').append('请刷新重试');
                }
            }
        })
    }
    function local(order) {
        ...
        if(type == 1){
            var url = "<?php echo site_url('orderlist/cashierOrderDetail'); ?>"+"?id="+order;
        }else {
            var url = "<?php echo site_url('orderlist/chargeOrderDetail'); ?>"+"?id="+order;
        }
