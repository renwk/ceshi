...
<div class="con_box me_list">
    <ul>
        <li>
            <span class="row_title">可用余额</span>
            <span class="roow_con" id="money_available"></span>
        </li>
        <li>
...
//        alert(card[1]);
        ...
        $.ajax({
            type: "post",
            url: "<?php echo site_url('usercard/requestMycardinfo'); ?>",
            data: data,
            dataType: "json",
            success: function (msg) {
                if(msg.msg == 'success'){
                    $('#showtable tr').remove();
                    $('#money_available').html(msg.data.mycard_info.money_available);
                    $('#status').html(msg.data.mycard_info.status);
                    $('#sname').html(msg.data.mycard_info.sname);
                    $('#time_pay').html(msg.data.mycard_info.time_pay);
                    $('#time_active').html(msg.data.mycard_info.time_active);
                    $('#time_validity').html(msg.data.mycard_info.time_validity);
                    $('#cname').html(msg.data.mycard_info.cname);
