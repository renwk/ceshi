 <style>
        .transfterdone{
            position: relative;
            padding-top:18px;
        }
        .transfterdone .letter{
            font-size:16px;
            color: #333;
        }
        .transfterdone .letter p:first-child{
            margin-bottom:10px;
        }
        .transfterdone .letter .tips{
            margin:12px 0 30px;
            font-size:14px;
            color: #999;
        }
        .transfterdone .line_row{
            overflow: hidden;
        }
        .transfterdone .line_row .line_name{
            line-height: 35px;
        }
        .transfterdone .line_row input{
            padding-left:12px;
            width:100%;
            line-height:35px;
            border:1px solid #999;
            border-radius: 6px;
            outline:none;
            font-size:18px;
            color: #333;
            box-sizing:border-box;
        }
        .transfterdone .line_row input.input_min{
            width:calc(100% - 92px);
            line-height:35px;
            font-size:15px;
        }
        .btn_wrap{
            padding-top: 40px;
            padding-bottom: 37.5px;
        }
        .send_code{
            position: absolute;
            right:0;
            top:0;
            background: #fff;
        }
        .send_code .btn{
            width:80px;
            text-align: center;
            padding: 9px 0px;
        }
        /*验证码按钮锁定*/
        .disabled_bg,
        .disabled_bg:hover,
        .disabled_bg:active{
            text-decoration: none !important;
            background: #ddd !important;
            color: #fff !important;
            border-color: #ddd !important;
        }
    </style>
<body class="p12">
<div class="con_box transfterdone">
    <div class="letter">
        <p><?php echo $wechat['nickname'];?>：</p>
        <p>您好！<br>
        <?php if( !empty($giveInfo) ):?>
            “<?php echo $sendWechat['nickname'];?>”于<?php echo date('Y-m-d H:i:s', $give['create_time'])?>赠送您价值<?php echo ib2Money( $giveInfo['accept_ib'] );?>元的iSpa泰美好的爱币，点击领取后可以在您的个人中心查看爱币余额。</p>
       <?php else:?>
       		您的好友未赠送您爱币噢</p>
       <?php endif;?>
        <p class="tips">注：爱币可以直接用于iSpa泰美好旗下门店消费。</p>
    </div>
     <?php if( !empty($giveInfo) ):?>
    <div class="line_row">
        <span class="line_name">领取手机号：<?php echo $user['userinfo']['mobile'];?></span>
    </div>
    <div class="btn_wrap">
        <a href="javascript:;" class="btn btn_big" onclick="getIb()">领取</a>
    </div>
    	<script>
    	var loadFlag = true;
			function getIb()
			{
				if ( !loadFlag ) {
					return false;
				} 
				loadFlag = false;
				var url = '<?php echo base_url('card/actionGetIb')?>';
				loadMod(url, {give_id: '<?php echo $give['id']?>'}, function(data){
					loadFlag = true;
					if(data.replace(/\s/g,'') == 'success'){
		                Xalert('领取成功');
		                setTimeout(href('<?php echo base_url('index/index')?>'), 3000);
		                return false;
					} else {
						Xalert(data, 1000);
						return false;
					}
					
				})
				

			}

    	</script>
    
      <?php endif;?>
</div>

<script>
    var validCode=true;
    //获取短信验证码
    function check_code(e)
    {

        /*验证手机号*/
        /*  var phone =  checkPhone();

         if(!phone) return false;*/

        var time=60;
        var $code=e;
        if (validCode){
            validCode=false;

            $code.addClass('disabled_bg');
            //获取短信验证码


            var t=setInterval(function(){
                time--;
                $code.html(time+" S");
                $code.addClass('disabled_bg');
                if(time==0){
                    clearInterval(t);
                    $code.html("重新获取");
                    $code.removeClass('disabled_bg');
                    validCode=true;
                }
                else {

                }
            },1000)
        }
    }
</script>
</body>
</html>